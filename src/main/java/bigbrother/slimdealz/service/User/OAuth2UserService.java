...
public class OAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String kakao_Id = kakaoUserInfo.getKakao_Id();
        String name = kakaoUserInfo.getName();

        // 소셜 ID 로 사용자를 조회, 없으면 socialId 와 이름으로 사용자 생성
        Optional<Member> byKakaoId = memberRepository.findByKakaoId(kakao_Id);
        Member member = byKakaoId.orElseGet(() -> saveSocialMember(kakao_Id, name));
    ...
    public Member saveSocialMember(String kakao_Id, String name) {
        log.info("--------------------------- saveSocialMember ---------------------------");
        Member newMember = Member.builder().kakao_Id(kakao_Id).name(name).role(MemberRole.USER).build();
